...
export async function upsertUserProfile(user) {
  ...
  const profilePayload = {
    ...
    totalXP: Math.max(0, Number(existingData?.totalXP) || 0),
    level: Math.max(1, Number(existingData?.level) || 1),
    trainerRank: typeof existingData?.trainerRank === 'string'
      ? existingData.trainerRank
      : 'Puppy Trainer',
  };
...
export async function isUsernameAvailable(username, currentUid = null) {
